...
除了浏览器缓存，网络请求在到达源服务器之前，还可能经过多层缓存：

1. **路由器缓存**
   - **DNS 缓存**：路由器会缓存 DNS 查询结果，减少 DNS 解析时间
   - **HTTP 缓存**：部分企业级路由器支持 HTTP 代理缓存，可以缓存 HTTP 响应
   - **特点**：缓存时间通常较短，主要用于局域网内多设备共享

2. **CDN 缓存（内容分发网络）**
   - **边缘节点缓存**：CDN 在全球各地部署边缘节点，缓存静态资源（HTML、CSS、JS、图片等）
   - **缓存策略**：根据 `Cache-Control` 响应头决定是否缓存
     - `public`：CDN 可以缓存
     - `private`：CDN 不缓存
     - `max-age`：控制缓存有效期
   - **优势**：
     - 就近访问：用户从最近的 CDN 节点获取资源，大幅减少延迟
     - 减轻源服务器压力：大部分请求由 CDN 节点响应，无需回源
     - 提升加载速度：CDN 节点通常带宽充足，传输速度快
   - **缓存更新**：
     - 通过文件版本号或 hash（如 `app.v1.2.3.js`）实现缓存更新
     - 使用 `Cache-Control: max-age` 控制缓存时间
     - 支持手动刷新 CDN 缓存（Purge）

3. **ISP 缓存**
   - 部分 ISP（互联网服务提供商）会在其网络中缓存热门资源
   - 透明代理缓存，用户通常无感知
...
前端性能优化可以从**性能指标、网络层面、渲染层面、代码层面、资源层面、构建层面**等多个维度进行优化。核心目标是**减少资源体积、加快加载速度、提升渲染性能、优化用户体验**。
...
| 指标（全称） | 解释 | 典型场景 |
|------------|------|---------|
| **FP (First Paint)** | **"屏幕亮了"**<br>浏览器开始渲染任何东西的时刻（哪怕只是背景色） | 屏幕从纯白变成浅灰色，虽然啥内容都没有，但你知道"它活着" |
| **FCP (First Contentful Paint)** | **"看到内容了"**<br>浏览器渲染出第一个内容（文字、图片、Logo）的时刻 | 页面上终于蹦出了"Loading..."文字或者导航栏 Logo |
| **LCP (Largest Contentful Paint)** ⭐ | **"主角登场"**<br>视口内可见的最大图片或文本块渲染完成的时刻。这是 Google 最看重的加载指标 | 淘宝详情页的大图终于刷出来了，你终于看清商品长啥样了 |

**性能标准（Google Core Web Vitals）：**

| 指标 | 优秀 (Good) | 需要改进 (Needs Improvement) | 较差 (Poor) |
|:--|:--|:--|:--|
| **FP** | < 1.0s | 1.0s - 2.5s | > 2.5s |
| **FCP** | < 1.8s | 1.8s - 3.0s | > 3.0s |
| **LCP** ⭐ | < 2.5s | 2.5s - 4.0s | > 4.0s |

> **说明**：以上标准基于 Google 的 Core Web Vitals 评估标准，其中 LCP、FID/INP、CLS 是直接影响 SEO 排名的三大核心指标。

---

### 2. Interaction（交互）：别卡得像 PPT！

东西加载出来了，用户开始点了，这时候最怕卡顿。这个阶段我们关注：

| 指标（全称） | 解释 | 典型场景 |
|------------|------|---------|
| **FID (First Input Delay)** ⭐ | **"第一下没反应？"**<br>用户第一次与页面交互（点击按钮、链接）到浏览器真正开始处理这个事件的时间差 | 兴奋地去点"登录"按钮，结果点了没反应，过了1秒钟按钮才变色 |
| **INP (Interaction to Next Paint)** | **"越用越卡？"**<br>FID的升级版。它不仅看第一下，还看你浏览全程中所有交互的延迟，取最慢的那几次 | 每输入一个字，输入框都要卡顿一下才能显示出来，有种"粘滞感" |
| **Long Task（长任务）** | **"谁在堵路？"**<br>任何执行时间超过50ms的JavaScript任务 | 主线程就像单行道，大卡车（复杂计算）一堵，后面的点击事件全得排队 |
...
> **注意**：INP 正在逐步取代 FID 作为 Core Web Vitals 的交互指标（2024年3月正式替换）。
...
这可能是最让人抓狂的体验。这个阶段我们关注：

| 指标（全称） | 解释 | 典型场景 |
|------------|------|---------|
| **CLS (Cumulative Layout Shift)** ⭐<br>累积布局偏移 | **"手滑点错了！"**<br>页面布局在加载过程中发生意外移动的程度 | 刚想点"取消"，顶部广告突然插队把页面往下挤，害你点成了"支付"。分数越低，页面越稳 |
...
### Web Vitals 核心指标（Google推荐）

**Core Web Vitals** 是Google定义的三个最重要的用户体验指标，直接影响SEO排名：

1. **LCP (Largest Contentful Paint)** - 加载性能 ⭐
2. **FID (First Input Delay)** / **INP (Interaction to Next Paint)** - 交互性能 ⭐
...
| 指标（全称） | 解释 | 说明 |
|------------|------|------|
| **TTFB (Time to First Byte)** | **首字节时间**<br>从请求发送到接收到服务器第一个字节的时间 | 反映服务器响应速度，及格线 < 100ms |
| **TTI (Time to Interactive)** | **可交互时间**<br>页面完全可交互的时间点（所有资源加载完成，主线程空闲） | 用户可以进行交互操作的时间，及格线 < 3.8s |
| **DCL (DOMContentLoaded)** | **DOM内容加载完成**<br>HTML文档完全加载和解析完成的时间 | DOM树构建完成，但资源可能仍在加载 |
| **L (Load)** | **页面完全加载**<br>页面所有资源（图片、样式表等）加载完成的时间 | load事件触发时间 |
| **FPS (Frames Per Second)** | **帧率**<br>每秒渲染的帧数 | 60fps为流畅，<30fps会感觉卡顿 |
